...
// ─────────────────────────────────────────────────────────────────────────────
// Base sanitizer for service / question keys
// ─────────────────────────────────────────────────────────────────────────────
// const strip = (s) =>
//   String(s || "")
//     .toLowerCase()
...
//   0) MATRIX question labels we need to mirror for adjustments
//      left: canonical normalized key → right: MATRIX "Question" label
//      Keep these labels EXACTLY as they appear in MATRIX.
// ──────────────────────────────────────────────────────────────────────────── */
// const MATRIX_QKEYS = {
//   "plumbing": {
//     "location": "where located",
...
// /* ────────────────────────────────────────────────────────────────────────────
//   1) Map resolved services → question groups
//      Keys here are already lower-cased (strip() does that).
//      Added robust aliases so callers can pass either label.
// ──────────────────────────────────────────────────────────────────────────── */
// const SERVICE_TO_QGROUP = {
//   // anchors → groups
//   "burst or leaking pipes": "plumbing",
...
//   "handyman": "handyman",
//   "handyman (general fixes)": "handyman", // alias → same group
...
//   "mobile mechanic": "mobile mechanic",
//   "car mechanic (general)": "mobile mechanic", // alias → same group

//   "car detailing (mobile)": "car detailing (mobile)",

//   "roadside service": "roadside service",
//   "tow truck / roadside assistance": "roadside service", // alias → same group
...
// /* ────────────────────────────────────────────────────────────────────────────
//   2) QUESTION & OPTION ALIASES
//      These map loose UI phrasings to canonical keys/options we use in code.
//      Keep canonical values equal to the MATRIX labels (for mirroring).
// ──────────────────────────────────────────────────────────────────────────── */
// export const QUESTION_ALIASES = {
//   /* ===================== CORE TRADES ===================== */
//   "plumbing": [
...
//   "car detailing (mobile)": [
//     // keep these exactly aligned with MATRIX_QKEYS mapping
//     [/^package$/i, "package"],
...
//     [/^leak$/i, "leak"],
//     [/^clog(ged)?$/i, "clogged"],
//     // severity (generic → concrete MATRIX labels)
//     [/^minor( leak| clog| drip)?$/i, () => "minor leak"],
//     [/^major( leak| clog)?$/i, () => "major leak"],
//     // access
//     [/^easy( access)?$/i, "easy access"],
...
//     .toLowerCase()
//     .replace(/\s+/g, " ")
//     // allow word chars, space, +, /, \, -, and parentheses
//     // (keeps keys like "car detailing (mobile)")
//     .replace(/[^\w\s/+\\\-()]/g, "")
//     .trim();
...
//     // severity (map to matching MATRIX option)
//     [/^minor( leak| clog| drip)?$/i, (m) =>
//       /\bclog\b/.test(m[0].toLowerCase()) ? "minor clog" : "minor leak"],
//     [/^major( leak| clog)?$/i, (m) =>
//       /\bclog\b/.test(m[0].toLowerCase()) ? "major clog" : "major leak"],

//     // access
//     [/^easy( access)?$/i, "easy access"],
//     [/^(behind|in) (wall|ceiling)$/i, (m) =>
//       (m[2] === "wall" ? "behind wall" : "behind ceiling")],
//   ],
