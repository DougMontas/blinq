...
function inferContentTypeFromBase64(b64) {
...
// Collect provider docs (base64 or URL) — builds links using req.baseUrl
function collectProviderDocs(req, user) {
  ...
  for (const d of fields) {
    const raw = d.value;
    if (!raw) continue;
    if (typeof raw === "string" && /^viewed@/i.test(raw)) continue; // skip sentinel ICA status

    let url;
    if (typeof raw === "string" && (raw.startsWith("http") || raw.startsWith("data:") || raw.startsWith("/uploads/"))) {
      url = absoluteUrl(req, raw);
    } else {
      // base64 or unknown → serve via our download endpoint
      url = absoluteUrl(req, `${base}/provider/${user._id}/documents/${d.key}/download`);
    }
...
// Download provider doc (base64 -> file or redirect)
router.get("/provider/:id/documents/:key/download", auth, checkAdmin, async (req, res) => {
  try {
    ...
    if (!raw || typeof raw !== "string") return res.status(404).send("Document not found");

    // URLs and /uploads are redirected
    ...
    // Otherwise treat as raw base64 (your Mongo values like JVBERi0..., iVBORw0..., /9j/...)
    const contentType = inferContentTypeFromBase64(raw);
...
// Email all provider docs to support
router.post("/provider/:id/documents/email", auth, checkAdmin, async (req, res) => {
  try {
    ...
    const rows = docs.map((d) => `<tr><td>${d.type}</td><td><a href="${d.url}">${d.filename}</a></td></tr>`).join("");
    ...
    await sendEmail({ to, subject: `Provider Documents – ${user.name || user.email}`, text, html });
