  ...
  try {
    ...
    if (provider.billingTier === "profit_sharing") {
      if (!provider.stripeCustomerId) {
        ...
        await provider.save();
        console.log(
          "✅ Stripe customer created for profit sharing:",
          customer.id
        );
      }
    ...
    if (provider.billingTier === "hybrid") {
      if (!paymentMethodId) {
        return res
          .status(400)
          .json({ msg: "Payment method ID is required for hybrid tier." });
      }
    ...
    return res
      .status(500)
      .json({ msg: "Subscription error", error: err.message });
  }
});
...
router.post("/create-payment-sheet", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Failed to initialize PaymentSheet.", error: err.message });
  }
});
