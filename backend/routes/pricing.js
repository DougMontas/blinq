...
// // ──────────────────────────────────────────────────────────────────────────────
// // Smart Estimate v2 (Non-breaking add-on)
// // Adds: Dev-only $1 test service (short-circuited)
// // ──────────────────────────────────────────────────────────────────────────────

// // ===== Config: anchors, NAICS per service, clamps, and weights =====
// const TEST_SERVICE_KEY = "Test: $1 Flat (No Fees)";
...
//     // not used for test but defined for clarity
//     [TEST_SERVICE_KEY]: [1, 1],
//   },
//   roundTo: 5,
//   cacheTTLms: 10 * 60 * 1000,
...
// // … all your dataset functions (_spv2_fetchJson, _spv2_geocodeToFips, _spv2_getACS, etc.) remain unchanged …
// // … multipliers & questionnaire remain unchanged …

// // Dynamic Service/Dispatch Fee
// function _spv2_dynamicServiceFee({ locM, compM, severity }) {
//   const sevBoost = severity === 'severe' ? 1.2 : severity === 'moderate' ? 1.0 : 0.9;
//   const raw = 100 * Math.pow(locM, 0.5) * Math.pow(compM, 0.5) * sevBoost;
//   return Math.max(100, _spv2_roundTo(raw, SPV2_CFG.roundTo));
...
//     // 🔧 Dev-only test service: short-circuit
//     if (service === TEST_SERVICE_KEY && process.env.NODE_ENV !== "production") {
//       const addr = (address || '').trim();
//       const addrLine = `${addr}${city ? ', ' + city : ''}${zipcode ? ' ' + zipcode : ''}`.trim();
//       return res.json({
//         ok: true,
...
//         lat: null,
//         lon: null,
//         flags: { testService: true },
//         breakdown: {
//           note: "Dev-only $1 test service, no multipliers or fees",
//         }
//       });
//     }

//     if (!(service in SPV2_SERVICE_ANCHORS)) {
...
//     // … normal geocode, datasets, multipliers, pricing logic here (unchanged) …
