...
// Requires: Node 18+ for global fetch. If you run older Node, install node-fetch
// and add: const fetch = require('node-fetch');
// Optional env: BEA_API_KEY to improve COL via BEA RPP; otherwise ACS fallback.
// ──────────────────────────────────────────────────────────────────────────────
...
const SPV2_SERVICE_ANCHORS = {
  ...
  // ===== The new endpoint: POST /v2/estimate =====
  router.post('/v2/estimate', async (req, res) => {
    try {
