...
export default function ProviderProfile() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        setExisting({
          ...
          independentContractorAgreement: data.independentContractorAgreement,
  ...
  const pickFile = async (key) => {
    try {
      ...
      if (res.type === "success") {
        setFiles((f) => ({ ...f, [key]: res }));
        Alert.alert("File Added", `${key} file was selected.`);
      }
    } catch {
      Alert.alert("Error", "File selection failed.");
    }
  ...
  const pickProfilePhoto = async () => {
    try {
      ...
      if (compressed) {
        setProfile((prev) => ({ ...prev, profilePicture: compressed }));
        Alert.alert("Success", "Profile photo added.");
      }
    } catch {
  ...
  const renderLink = (label, url) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={{ marginTop: 4 }}
      >
        {/* <Text style={{ color: "#0066cc" }}> {label}</Text> */}
      </TouchableOpacity>
    );
  };
  ...
  return (
    <ScrollView contentContainerStyle={s.container}>
      <BackButton />
      <Text style={s.header}>Complete Your Profile</Text>
      <Text style={{textAlign: 'center', fontSize: 12, color: 'red'}}>* All fields are required for activation.</Text>
      ...
      {[
        ...
        { label: "Extracted Zipcode", key: "zipcode", editable: false },
        { label: "Service Coverage Zipcode", key: "serviceZipcode" },
        {
          label: "Last 4 of SSN",
          key: "ssnLast4",
          keyboardType: "number-pad",
      ...
      {Object.entries({
        ...
        "Proof of Insurance": "proofOfInsurance",
        "Contractor Agreement": "independentContractorAgreement",
      }).map(([label, key]) => (
        <View key={key}>
          {renderLink(label, existing[key])}
          <TouchableOpacity style={s.uploadBtn} onPress={() => pickFile(key)}>
            <Text style={s.uploadBtnText}>Upload {label}</Text>
          </TouchableOpacity>
          {files[key] && (
            <Text style={s.fileText}>Selected: {files[key].name}</Text>
          )}
        </View>
      ))}
