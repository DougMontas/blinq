...
export default function ProviderJobStatus() {
          ...
          content: {
            ...
            body:
              Number.isFinite(Number(amount)) && Number(amount) > 0
                ? `Customer paid $${Number(amount).toFixed(
                    2
                  )}. You can proceed.`
  ...
  const handleCallCustomer = useCallback(async () => {
    ...
    const url =
      Platform.OS === "ios" ? `telprompt:${sanitized}` : `tel:${sanitized}`;
    try {
    ...
    const fetchJob = async () => {
      try {
        ...
        if (wasAwaiting && !nowAwaiting && isAdditionalPaid(data)) {
          ...
          if (paidBannerTimerRef.current)
            clearTimeout(paidBannerTimerRef.current);
          paidBannerTimerRef.current = setTimeout(
              ...
              <View style={styles.paidBanner}>
                ...
                >
                  ...
                  <Text style={styles.paidBannerText}>
                    {hasAddon
                      ? `Customer paid $${Number(job.additionalCharge).toFixed(
                          2
            ...
            <View style={styles.workflowContainer}>
              ...
              <View style={styles.stepCard}>
                ...
                <TouchableOpacity
                  style={[
                    styles.actionButton,
                    job.arrivalImage && styles.actionButtonCompleted,
                  ]}
                  onPress={() => pickAndUpload("arrival")}
                  disabled={awaitingAdditional}
                >
                  <Camera
                    color={job.arrivalImage ? "#22c55e" : "#fff"}
                    size={20}
                  />
                  <Text
                    style={[
              ...
              <View style={styles.stepCard}>
                <View style={styles.stepHeader}>
                  ...
                  <Text style={styles.stepTitle}>
                    Step 2: Add-ons & Charges (Optional)
                  </Text>
                ...
                <Text style={styles.stepDescription}>
                  If required, add any additional charges for parts or labor not
                ...
                {!hasAddon || awaitingAdditional ? (
                  <>
                    ...
                    <TouchableOpacity
                      style={[
                        styles.actionButton,
                        awaitingAdditional && styles.actionButtonDisabled,
                      ]}
                  ...
                  <View style={styles.paidLine}>
                    ...
                    <Text style={styles.paidLineText}>
                      ...
                      {Number(job.additionalCharge).toFixed(2)} received.
                    </Text>
                  </View>
                )}
              </View>
              ...
              <View style={styles.stepCard}>
                ...
                <Text style={styles.stepDescription}>
                  Capture a photo showing the completed work.
                </Text>
                <TouchableOpacity
                  style={[
                    styles.actionButton,
                    job.completionImage && styles.actionButtonCompleted,
                  ]}
                  onPress={() => pickAndUpload("completion")}
                  disabled={awaitingAdditional}
                >
                  <Camera
                    color={job.completionImage ? "#22c55e" : "#fff"}
                    size={20}
                  />
                  <Text
                    style={[
              ...
              <View style={styles.stepCard}>
                ...
                <Text style={styles.stepDescription}>
                  Once all work is done and photos are uploaded, mark the job as
                  completed to notify the customer.
                </Text>
                <TouchableOpacity
                  style={[
                    styles.primaryButton,
                    (!job.arrivalImage || !job.completionImage) &&
