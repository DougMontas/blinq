...
//               <Image
//                 source={
//                   typeof img === "string" && img ? { uri: img } : ComingSoon
//                 }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      >
        <Text style={styles.heroText}>
          Hi {firstName},
          <Text style={styles.heroSub}> what kind of handyman work do you need?</Text>
        </Text>
        <TouchableOpacity style={styles.ctaBtn}>
        ...
        {SUBCATEGORIES.map(({ label, img }) => {
          ...
          return (
            <TouchableOpacity
              ...
              style={[styles.card, !isEnabled && styles.disabledCard]}
              onPress={() =>
                isEnabled ? handleSelect(label) : Alert.alert("Coming Soon", `${label} is not yet available.`)
              }
            >
              <Image
              ...
              />
              <View style={styles.cardBody}>
                <Text style={styles.cardLabel}>{label}{!isEnabled && " (Coming Soon)"}</Text>
              </View>
            </TouchableOpacity>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  ctaText: { color: "red", fontWeight: "600", fontStyle: "italic", textAlign: "center" },
  sectionTitle: { fontSize: 22, fontWeight: "700", textAlign: "center", marginTop: 12 },
  cardsWrap: { flexDirection: "row", flexWrap: "wrap", justifyContent: "center", paddingHorizontal: 8 },
  card: {
    width: 160,
    margin: 8,
    backgroundColor: "#f9f9f9",
