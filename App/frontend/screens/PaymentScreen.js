...
export default function PaymentScreen() {
  ...
  const handlePay = async () => {
    if (!paymentReady) return Alert.alert("Payment not ready", "Please wait while we prepare your payment.");
    try {
      const { error } = await presentPaymentSheet();
      if (error) return Alert.alert("Payment Failed", error.message || "Unknown error occurred.");
  ...
  if (showAnimation) {
    return (
      <LoadingScreen
        ...
        holdDuration={400}
        exitDuration={800}
        onAnimationEnd={() => navigation.navigate("CustomerJobStatus", { jobId })}
      />
    );
  }
...
//       await api.put(`/jobs/complete-payment/${jobId}`);
//       navigation.replace("CustomerJobStatus", { jobId }); // navigate directly
//     } catch (err) {
//       console.error("Payment handling error:", err);
//       Alert.alert("Error", "Failed to complete payment.");
//     }
