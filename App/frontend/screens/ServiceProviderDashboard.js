...
// ServiceProviderDashboard with pending jobs integration (full code)
...
function computeCohort(u) {
  const vals = [
    ...
    .filter(Boolean)
    .map((v) => String(v).toLowerCase());
  if (vals.some((s) => s.includes("hybrid") || s.includes("pro") || s.includes("premium"))) return "hybrid";
  if (vals.some((s) => s.includes("profit"))) return "profit_sharing";
  return "profit_sharing";
}
...
export default function ServiceProviderDashboard() {
  ...
  const presentInviteNotification = useCallback(async (payload, cohort) => {
    const jobId = payload?.jobId || payload?._id || "";
    const clickable = resolveClickable(payload || {}, cohort);
    const title = clickable ? "New Job Invitation" : "New Job Nearby";
    const body = clickable ? "A new customer needs you. Tap to open the invite." : "You’ve received a job invite. Open the app to view details.";
    try {
      await Notifications.scheduleNotificationAsync({
        content: { title, body, sound: "default", data: { jobId, clickable } },
        trigger: null,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const zipRaw = u?.serviceZipcode ?? (Array.isArray(u?.zipcode) ? u.zipcode[0] : u?.zipcode) ?? "";
      const svcRaw = (u?.serviceType || "").trim();
      const zip = encodeURIComponent(zipRaw);
      const svc = encodeURIComponent(svcRaw);
      let invites = [];
      try {
        const { data } = await api.get(`/jobs/pending?serviceType=${svc}&serviceZipcode=${zip}`);
        const now = new Date();
        invites = (data || []).filter(
          (j) => j.invitationExpiresAt && new Date(j.invitationExpiresAt) > now && !j.acceptedProvider && !(j.declinedBy || []).includes(u._id)
        );
      } catch (e) {
        if (e?.response?.status !== 404) throw e;
        invites = [];
  ...
  useEffect(() => { ensureNotificationSetup(); }, [ensureNotificationSetup]);
...
const styles = StyleSheet.create({
  ...
  profileTitle: { fontSize: 16, fontWeight: "bold", color: "#fff" },
  profileSubtitle: { fontSize: 14, color: "#e0e7ff", marginTop: 4 },
  mapCard: { borderRadius: 16, padding: 20, marginBottom: 20, borderWidth: 1, borderColor: "rgba(255,255,255,0.1)" },
  cardHeader: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 16 },
  cardTitle: { fontSize: 18, fontWeight: "bold", color: "#fff" },
  mapSolid: { height: 260, borderRadius: 12 },
  section: { marginTop: 40, marginBottom: 40 },
