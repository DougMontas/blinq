...
/**
 * CustomerJobStatus.js (Updated)
 * - Shows service provider on map after accepting job
 * - Adds ETA and polyline route between service pro and customer
 * - Zooms to fit both markers
 */
  ...
  useEffect(() => {
    ...
    socket.on("locationUpdate", ({ provider }) => {
      ...
      if (job?.location?.coordinates) {
        ...
        const toRad = (val) => (val * Math.PI) / 180;
        const R = 6371;
        const dLat = toRad(lat2 - coords.latitude);
        const dLon = toRad(lon2 - coords.longitude);
        const a =
          Math.sin(dLat / 2) ** 2 +
        ...
        const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
        const distance = R * c;
        setEta(Math.round((distance / 0.6) * 60));
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {(jobLocation.latitude && jobLocation.longitude) && (
        ...
        >
          ...
          {routeCoords.length === 2 && (
            <Polyline coordinates={routeCoords} strokeColor="#1976d2" strokeWidth={3} />
          )}
        </MapView>
      )}
