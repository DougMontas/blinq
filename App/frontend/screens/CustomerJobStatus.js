...
export default function CustomerJobStatus() {
  ...
  const handleCustomerComplete = async () => {
    ...
    try {
      const { data } = await api.put(`/jobs/${jobId}/complete/customer`);
      navigation.navigate("RateProvider", { jobId });
    } catch {
      Alert.alert("Error", "Could not confirm completion");
    } finally {
      setConfirming(false);
  ...
  const handleNotComplete = () => {
    Alert.alert("Reported", "We’ve noted the job is not complete.");
  };

  const handleDispute = () => {
    Alert.alert("Dispute Submitted", "Our support team has been notified.");
    api.post(`/jobs/${jobId}/dispute`);
  };

  const handleCancelJob = async () => {
    try {
      await api.put(`/jobs/${jobId}/cancelled`, { cancelledBy: "customer" });
      Alert.alert("Cancelled", "The job has been cancelled.");
    } catch {
      Alert.alert("Error", "Failed to cancel job.");
    }
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {providerInfo && job.status === "accepted" && (
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Your Service Pro</Text>
          {providerInfo.profilePictureUrl && (
            <Image source={{ uri: providerInfo.profilePictureUrl }} style={{ width: 160, height: 160, borderRadius: 100 }} />
          )}
          <Text style={{ fontWeight: "bold", fontSize: 16 }}>{providerInfo.name}</Text>
          <Text>{providerInfo.businessName}</Text>
          <Text>{providerInfo.aboutMe}</Text>
      ...
      {job.arrivalImage && (
        <Image source={{ uri: `data:image/jpeg;base64,${job.arrivalImage.toString("base64")}` }} style={{ width: "100%", height: 200, marginBottom: 10 }} resizeMode="cover" />
      )}

      {job.completionImage && (
        <Image source={{ uri: `data:image/jpeg;base64,${job.completionImage.toString("base64")}` }} style={{ width: "100%", height: 200, marginBottom: 10 }} resizeMode="cover" />
      )}
      ...
      {job.providerCompleted && !job.customerCompleted && (
        <View style={styles.confirm}>
          ...
          <TouchableOpacity style={[styles.confirmButton, confirming && styles.confirmButtonDisabled]} onPress={handleCustomerComplete} disabled={confirming}>
            <Text style={styles.confirmButtonText}>{confirming ? "Confirming…" : "Confirm Job Complete"}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.confirmButton} onPress={handleNotComplete}>
