...
// //Added all categories >> //current working
// import React, { useEffect, useState } from "react";
// import {
//   ScrollView,
//   View,
...
/**
 * Categories must match backend category keys:
 * - Plumbing, Roofing, HVAC, Electrician, Handyman, Locksmith,
 * - Cleaning, Auto, Pest Control, Painting, Landscaping, Consulting/Estimating
 * (Auto umbrella covers: Mobile Mechanic, Roadside Service, Car Detailing)
 */
const categories = [
  { name: "Plumbing", icon: Droplets, color: "#60a5fa" },
  { name: "Roofing", icon: Home, color: "#f87171" },
  { name: "HVAC", icon: Wind, color: "#67e8f9" },
  { name: "Electrician", icon: Zap, color: "#facc15" },
  { name: "Handyman", icon: Wrench, color: "#818cf8" }, // no nested flow
  { name: "Locksmith", icon: Key, color: "#fbbf24" },
  { name: "Cleaning", icon: Scissors, color: "#f472b6" },
  { name: "Auto", icon: Car, color: "#93c5fd" }, // umbrella for Mobile Mechanic / Roadside / Detailing
  { name: "Pest Control", icon: Bug, color: "#22c55e" },
  { name: "Painting", icon: Paintbrush, color: "#e879f9" },
  { name: "Landscaping", icon: Leaf, color: "#34d399" },
          ...
          <View style={styles.cardsWrap}>
            {categories.map(({ name, icon: Icon, color, fullWidth }) => (
              ...
              >
                ...
                {!fullWidth && <ChevronRight color="rgba(255,255,255,0.3)" size={20} />}
              </TouchableOpacity>
            ))}
          </View>
